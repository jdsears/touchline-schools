...
import { normalizePlayerPositions, normalizePositions } from '../utils/playerUtils.js'
...
const BADGE_TYPES = {
  potm: { title: 'Player of the Match', icon: '⭐', description: 'Outstanding match performance' },
  ...
  team_player: { title: 'Team Player', icon: '🤝', description: 'Great teamwork and support' },
...
router.get('/:id', authenticateToken, async (req, res, next) => {
  try {
    ...
    // Verify the player belongs to the user's team (managers/assistants)
    // or the user is linked to this player (parents/players)
    const player = result.rows[0]
    if (player.team_id !== req.user.team_id && player.id !== req.user.player_id && !req.user.is_admin) {
      return res.status(403).json({ message: 'Access denied' })
    }

    // Normalize positions from JSONB format to simple string array
    res.json(normalizePlayerPositions(player))
  } catch (error) {
    next(error)
  }
})
...
router.put('/:id', authenticateToken, async (req, res, next) => {
  try {
    ...
    const playerCheck = await pool.query('SELECT team_id FROM players WHERE id = $1', [id])
  ...
  try {
    ...
    const result = await pool.query(
      `SELECT o.*, u.name as observer_name,
       ...
       WHERE o.player_id = $1 ORDER BY o.created_at DESC`,
      [id]
    )
...
router.post('/:id/observations', authenticateToken, async (req, res, next) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO observations (player_id, observer_id, type, content, context, context_type, match_id, training_session_id)
...
router.put('/:id/observations/:obsId', authenticateToken, async (req, res, next) => {
  try {
    ...
    // Verify the observation belongs to this player
    const existing = await pool.query(
      'SELECT * FROM observations WHERE id = $1 AND player_id = $2',
      [obsId, id]
    )
    ...
    const result = await pool.query(
      `UPDATE observations SET
       ...
       WHERE id = $7 AND player_id = $8 RETURNING *`,
...
router.delete('/:id/observations/:obsId', authenticateToken, async (req, res, next) => {
  try {
    ...
    const result = await pool.query(
      'DELETE FROM observations WHERE id = $1 AND player_id = $2 RETURNING id',
      [obsId, id]
    )
...
router.get('/:id/idp', authenticateToken, async (req, res, next) => {
  try {
    ...
    // Verify the player belongs to the user's team or is the user's linked player
    const playerCheck = await pool.query('SELECT team_id FROM players WHERE id = $1', [id])
    if (playerCheck.rows.length === 0) {
      return res.status(404).json({ message: 'Player not found' })
    }
    if (playerCheck.rows[0].team_id !== req.user.team_id && id !== req.user.player_id && !req.user.is_admin) {
      return res.status(403).json({ message: 'Access denied' })
    }

    const result = await pool.query(
      'SELECT * FROM development_plans WHERE player_id = $1 ORDER BY created_at DESC LIMIT 1',
      [id]
    )
...
router.patch('/:id/idp/settings', authenticateToken, async (req, res, next) => {
  try {
    ...
    // Find the latest IDP for this player
    const existing = await pool.query(
      'SELECT id, created_at FROM development_plans WHERE player_id = $1 ORDER BY created_at DESC LIMIT 1',
      [id]
    )
    if (existing.rows.length === 0) {
      return res.status(404).json({ message: 'No development plan found for this player' })
    }
...
router.post('/:id/idp/generate', authenticateToken, async (req, res, next) => {
  try {
    ...
    // Get player with team age group
    const playerResult = await pool.query(
      'SELECT p.*, t.age_group FROM players p LEFT JOIN teams t ON p.team_id = t.id WHERE p.id = $1',
      [id]
    )
    ...
    // Verify the player belongs to the user's team
    if (player.team_id !== req.user.team_id && !req.user.is_admin) {
      return res.status(403).json({ message: 'Access denied' })
    }
    ...
    const idpUsageCheck = await checkAndIncrementUsage(player.team_id, 'idp', entitlements)
    if (!idpUsageCheck.allowed) {
      return res.status(429).json({
        message: idpUsageCheck.limit === 0
          ? 'Individual Development Plans are not available on your current plan. Upgrade to Core or Pro to generate IDPs for your players.'
    ...
    const obsResult = await pool.query(
      'SELECT type, content FROM observations WHERE player_id = $1 ORDER BY created_at DESC LIMIT 20',
      [id]
    )
    ...
    const stream = await generatePlayerIDP(player, obsResult.rows, weeks)
    let fullText = ''
    ...
    if (fullText) {
      ...
      const result = await pool.query(
        `INSERT INTO development_plans (player_id, goals, strengths, areas_to_improve, notes, review_weeks, next_review_at, auto_review)
...
router.post('/:id/attributes/analyze', authenticateToken, async (req, res, next) => {
  try {
    ...
    const obsResult = await pool.query(
      'SELECT type, content, context FROM observations WHERE player_id = $1 ORDER BY created_at DESC LIMIT 20',
      [id]
    )

    // Also get video clip feedback tagged to this player
    ...
    try {
      const clipResult = await pool.query(
        `SELECT t.feedback, t.rating, c.clip_type, c.title
         ...
         WHERE t.player_id = $1 AND t.feedback IS NOT NULL
    ...
    const [extractedAttributes, analysisText] = await Promise.all([
      extractPlayerAttributes(player, allObservations),
      analyzePlayerAttributes(player, allObservations),
    ])
...
router.post('/:id/invite-parent', authenticateToken, async (req, res, next) => {
  try {
    ...
    // Get player to verify it exists and get team_id
    const playerResult = await pool.query(
      'SELECT p.id, p.name, p.team_id, t.name as team_name FROM players p JOIN teams t ON p.team_id = t.id WHERE p.id = $1',
      [id]
    )
    ...
    const result = await pool.query(
      `INSERT INTO invites (team_id, email, role, token, expires_at, player_id)
       VALUES ($1, $2, 'parent', $3, $4, $5) RETURNING id, email, role, created_at, expires_at, player_id`,
      [player.team_id, email, token, expiresAt, id]
    )

    // Generate invite link (always return it for manual sharing)
    const inviteLink = `${getFrontendUrl()}/invite/${token}`
    console.log(`Parent invite link for ${email} (player: ${player.name}): ${inviteLink}`)
...
// Get pending parent invites for a player
  ...
  try {
    ...
    const result = await pool.query(
      `SELECT id, email, role, created_at, expires_at
       FROM invites
       WHERE player_id = $1 AND expires_at > NOW() AND accepted_at IS NULL
...
// ==========================================
// Player Zone API (for players/parents)
// ==========================================

// Get all player zone data in one call
router.get('/:id/zone', authenticateToken, async (req, res, next) => {
  try {
    ...
    // Verify user has access (team member, linked player, or admin)
    if (player.team_id !== req.user.team_id && id !== req.user.player_id && !req.user.is_admin) {
      return res.status(403).json({ message: 'Access denied' })
    }
    ...
    // Get upcoming matches (include prep_notes for player match prep view)
    const upcomingMatchesResult = await pool.query(
      `SELECT id, opponent, date, is_home, location, notes, meetup_time, meetup_location, prep_notes,
              ...
              (SELECT status FROM match_availability WHERE match_id = matches.id AND player_id = $2) as my_availability
       FROM matches
    ...
    // Get recent matches (include report, videos, notes for player view)
    ...
    const trainingResult = await pool.query(
      `SELECT id, date, time, focus_areas, duration, notes, location, session_type, meet_time,
              ...
              (SELECT status FROM training_availability WHERE session_id = training_sessions.id AND player_id = $2) as my_availability
    ...
    const observationsResult = await pool.query(
      `SELECT o.type, o.content, o.context, o.context_type, o.created_at,
       ...
       LEFT JOIN matches m ON o.match_id = m.id
       LEFT JOIN training_sessions ts ON o.training_session_id = ts.id
       WHERE o.player_id = $1 AND o.context_type IN ('match', 'training')
    ...
    // Get team videos visible to players
    ...
    // Get Player of the Match awards
    ...
    try {
      ...
      console.error('Failed to load player achievements:', err.message)
    }
...
// Update match availability (for player/parent)
router.post('/:id/availability/:matchId', authenticateToken, async (req, res, next) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO match_availability (match_id, player_id, status, notes, updated_at)
       VALUES ($1, $2, $3, $4, NOW())
...
// Update training availability (for player/parent)
router.post('/:id/training-availability/:sessionId', authenticateToken, async (req, res, next) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO training_availability (session_id, player_id, status, notes, updated_at)
       VALUES ($1, $2, $3, $4, NOW())
...
router.get('/:id/achievements', authenticateToken, async (req, res, next) => {
  try {
    ...
    console.log(`[Achievements] Loaded ${result.rows.length} achievements for player ${id}`)
    res.json(result.rows)
  } catch (error) {
    // If table doesn't exist, return empty array instead of crashing
    if (error.code === '42P01') {
      console.error('[Achievements] player_achievements table does not exist - run migrations')
      return res.json([])
    }
...
router.post('/:id/achievements', authenticateToken, async (req, res, next) => {
  try {
    ...
    // Create achievement
    const result = await pool.query(
      `INSERT INTO player_achievements (player_id, achievement_type, title, description, icon, match_id, training_session_id, awarded_by)
    ...
    console.log(`[Achievements] Created achievement ${achievement.id} for player ${id}: ${title}`)
      ...
      for (const contact of parentContacts) {
        ...
        if (userId) {
          ...
          notifParams.push(
            ...
            player.team_id,
            'achievement',
            `🎉 ${player.name} earned: ${title}!`,
            description || `${player.name} has been awarded the "${title}" badge.`,
            JSON.stringify({
              achievement_id: achievement.id,
              achievement_type,
              icon: achievement.icon,
    ...
    if (error.code === '42P01') {
      console.error('player_achievements table does not exist - run migrations')
...
router.delete('/:id/achievements/:achievementId', authenticateToken, async (req, res, next) => {
  try {
    ...
    const result = await pool.query(
      'DELETE FROM player_achievements WHERE id = $1 AND player_id = $2 RETURNING id',
      [achievementId, id]
    )
