...
const uploadsBaseDir = path.join(__dirname, '../uploads/schools')

const complianceStorage = multer.diskStorage({
  destination: (req, file, cb) => {
    const { schoolId } = req.params
    const userId = req.body.user_id || req.params.id
    const dir = path.join(uploadsBaseDir, schoolId, 'compliance', String(userId))
...
router.get('/:schoolId/compliance/overview', authenticateToken, loadSchool, requireSchoolFeature('safeguarding'), requireSchoolRole('owner', 'admin', 'secretary'), async (req, res, next) => {
  try {
    ...
    const [
      ...
      pool.query(
        'SELECT COUNT(DISTINCT user_id) FROM compliance_records WHERE school_id = $1',
        [schoolId]
      ),
      ...
      pool.query(
        ...
        FROM compliance_records WHERE school_id = $1`,
        [schoolId, thirtyDaysFromNow, now]
      ),
      ...
      pool.query(
        `SELECT COUNT(*) FROM safeguarding_roles
         WHERE school_id = $1 AND role_type = 'welfare_officer'`,
        [schoolId]
      ),
      // Active alerts
      pool.query(
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO compliance_records (
        school_id, user_id, role_at_club,
        dbs_number, dbs_issue_date, dbs_expiry_date, dbs_status, dbs_update_service,
        first_aid_cert_type, first_aid_expiry,
        safeguarding_course, safeguarding_date, safeguarding_expiry,
        coaching_badges,
...
router.post('/:schoolId/compliance/:id/documents', authenticateToken, loadSchool, requireSchoolFeature('safeguarding'), requireSchoolRole('owner', 'admin', 'secretary'), upload.single('document'), async (req, res, next) => {
  try {
    ...
    const record = await pool.query(
      'SELECT * FROM compliance_records WHERE id = $1 AND school_id = $2',
      [id, schoolId]
    )
    ...
    const storageKey = `schools/${schoolId}/compliance/${record.rows[0].user_id}/${req.file.filename}`
    ...
    const result = await pool.query(
      `UPDATE compliance_records
       ...
       RETURNING *`,
      [JSON.stringify(documentEntry), id, schoolId]
    )
...
router.post('/:schoolId/safeguarding/roles', authenticateToken, loadSchool, requireSchoolFeature('safeguarding'), requireSchoolRole('owner', 'admin', 'secretary'), async (req, res, next) => {
  try {
    ...
    // Verify the user is a school member
    const memberCheck = await pool.query(
      'SELECT id FROM school_members WHERE school_id = $1 AND user_id = $2 AND status = $3',
      [schoolId, user_id, 'active']
    )

    if (memberCheck.rows.length === 0) {
      return res.status(400).json({ error: 'User must be an active school member' })
    }

    // Check for existing assignment of the same role type to same user
    const existing = await pool.query(
      'SELECT id FROM safeguarding_roles WHERE school_id = $1 AND user_id = $2 AND role_type = $3',
      [schoolId, user_id, role_type]
    )
    ...
    const result = await pool.query(
      `INSERT INTO safeguarding_roles (
        school_id, user_id, role_type, qualifications, dbs_number, dbs_expiry, assigned_by
      ) VALUES ($1,$2,$3,$4,$5,$6,$7)
      RETURNING *`,
      [schoolId, user_id, role_type, qualifications || null, dbs_number || null, dbs_expiry || null, req.user.id]
    )
      ...
      try {
        ...
        if (userResult.rows.length > 0) {
          ...
          await sendNotificationEmail(assignedUser.email, {
            teamName: req.school.name,
            title: `Safeguarding Role Assigned: ${roleLabel}`,
            message: `You have been assigned the role of ${roleLabel} at ${req.school.name}. This is an important responsibility for the welfare of all members.`,
          })
        }
      } catch (emailErr) {
...
router.delete('/:schoolId/safeguarding/roles/:id', authenticateToken, loadSchool, requireSchoolFeature('safeguarding'), requireSchoolRole('owner', 'admin', 'secretary'), async (req, res, next) => {
  try {
    ...
    const result = await pool.query(
      'DELETE FROM safeguarding_roles WHERE id = $1 AND school_id = $2 RETURNING *',
      [id, schoolId]
    )
      ...
      try {
        const welfareOfficers = await pool.query(
          `SELECT u.email, u.name FROM safeguarding_roles sr
           ...
           WHERE sr.school_id = $1 AND sr.role_type = 'welfare_officer'`,
          [schoolId]
        )
        ...
        for (const officer of welfareOfficers.rows) {
          await sendNotificationEmail(officer.email, {
            ...
            message: `A new safeguarding incident (severity: ${severity || 'medium'}) has been reported at ${req.school.name}. Please review it as soon as possible.`,
          })
        }

        // Also notify school owner
        const owners = await pool.query(
          `SELECT u.email FROM school_members cm
  ...
  try {
    ...
    await pool.query(
      `UPDATE safeguarding_incidents
       ...
       WHERE id = $2 AND school_id = $3`,
      [JSON.stringify(viewEntry), id, schoolId]
    )
...
router.put('/:schoolId/safeguarding/incidents/:id', authenticateToken, loadSchool, requireSchoolFeature('safeguarding'), requireSchoolRole('owner', 'admin', 'treasurer', 'secretary', 'coach'), requireSafeguardingAccess, async (req, res, next) => {
  try {
    ...
    const existing = await pool.query(
      'SELECT id FROM safeguarding_incidents WHERE id = $1 AND school_id = $2',
      [id, schoolId]
    )
...
router.put('/:schoolId/compliance/alerts/:id', authenticateToken, loadSchool, requireSchoolFeature('safeguarding'), requireSchoolRole('owner', 'admin', 'secretary'), async (req, res, next) => {
  try {
    ...
    const result = await pool.query(
      ...
      RETURNING *`,
      [status, notes, req.user.id, id, schoolId]
    )
  ...
  try {
    ...
    async function createAlertIfNew(alertType, targetUserId, message, severity) {
      const existingQuery = targetUserId
        ? `SELECT id FROM compliance_alerts WHERE school_id = $1 AND alert_type = $2 AND target_user_id = $3 AND status = 'active'`
        : `SELECT id FROM compliance_alerts WHERE school_id = $1 AND alert_type = $2 AND target_user_id IS NULL AND status = 'active'`

      const existingParams = targetUserId
        ? [schoolId, alertType, targetUserId]
        : [schoolId, alertType]
      ...
      const insertResult = await pool.query(
        `INSERT INTO compliance_alerts (school_id, alert_type, target_user_id, message, severity, status)
         VALUES ($1,$2,$3,$4,$5,'active')
         RETURNING *`,
        [schoolId, alertType, targetUserId || null, message, severity || 'medium']
      )
    ...
    const dbsExpiring = await pool.query(
      `SELECT cr.user_id, u.name FROM compliance_records cr
       LEFT JOIN users u ON cr.user_id = u.id
       WHERE cr.school_id = $1 AND cr.record_type = 'dbs'
         AND cr.expiry_date > $2 AND cr.expiry_date <= $3`,
      [schoolId, now, thirtyDaysFromNow]
    )
    ...
    const firstAidExpiring = await pool.query(
      `SELECT cr.user_id, u.name FROM compliance_records cr
       LEFT JOIN users u ON cr.user_id = u.id
       WHERE cr.school_id = $1 AND cr.record_type = 'first_aid'
         AND cr.expiry_date > $2 AND cr.expiry_date <= $3`,
      [schoolId, now, thirtyDaysFromNow]
    )
    ...
    const trainingExpiring = await pool.query(
      `SELECT cr.user_id, u.name FROM compliance_records cr
       LEFT JOIN users u ON cr.user_id = u.id
       WHERE cr.school_id = $1 AND cr.record_type = 'safeguarding_training'
         AND cr.expiry_date > $2 AND cr.expiry_date <= $3`,
      [schoolId, now, thirtyDaysFromNow]
    )
    ...
    if (parseInt(welfareCheck.rows[0].count) === 0) {
      const alert = await createAlertIfNew(
        ...
        'No welfare officer is assigned to this school. This is a mandatory safeguarding requirement.',
        'high'
      )
      if (alert) alertsCreated.push(alert)
    }
    ...
    const membersWithoutDbs = await pool.query(
      `SELECT cm.user_id, u.name FROM school_members cm
       JOIN users u ON cm.user_id = u.id
       WHERE cm.school_id = $1 AND cm.role IN ('coach', 'owner', 'admin') AND cm.status = 'active'
