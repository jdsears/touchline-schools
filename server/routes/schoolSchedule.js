...
// ==========================================
// HELPER: Load team and its school context
// ==========================================
...
async function loadTeamAndClub(req, res, next) {
  try {
    ...
    const result = await pool.query(
      `SELECT t.*, c.id AS school_id, c.name AS club_name, c.subscription_tier,
              c.stripe_account_id, c.settings AS club_settings
       FROM teams t
       LEFT JOIN schools c ON t.school_id = c.id
       WHERE t.id = $1`,
      [teamId]
    )
    ...
    // Set school context for middleware compatibility
    if (team.school_id) {
      req.school = {
        id: team.school_id,
        name: team.club_name,
      ...
      req.params.schoolId = team.school_id
    }
  ...
  try {
    ...
    const parentResult = await pool.query(
      `INSERT INTO session_schedule (
        school_id, team_id, session_type, title, date, start_time, end_time,
        venue_name, venue_address, meet_time,
        opponent, is_home, match_id,
        is_recurring, recurrence_rule,
      ...
      RETURNING *`,
      [
        schoolId, teamId, session_type, title || null, date, start_time, end_time || null,
    ...
    if (is_recurring && recurrence_rule) {
      ...
      for (const sess of recurringDates) {
        const childResult = await pool.query(
          ...
          RETURNING *`,
          [
            schoolId, teamId, session_type, sess.title, sess.date, start_time, sess.end_time,
    ...
    if (isEmailEnabled() && schoolId) {
      try {
        // Get all guardians linked to pupils on this team
        ...
        for (const guardian of guardiansResult.rows) {
          await sendEmail(guardian.email, 'sessionCreated', {
            clubName: req.school?.name || teamName,
            teamName,
            sessionTitle,
...
router.delete('/:teamId/schedule/:id', authenticateToken, loadTeamAndClub, async (req, res, next) => {
  try {
    ...
    // Send cancellation email to guardians of pupils who responded
    if (isEmailEnabled() && req.team.school_id) {
      try {
        const respondersResult = await pool.query(
          `SELECT DISTINCT g.email, g.first_name
           FROM availability_responses ar
           JOIN pupils p ON ar.pupil_id = p.id
           JOIN pupil_guardians pg ON pg.pupil_id = p.id
        ...
        for (const guardian of allGuardians) {
          await sendEmail(guardian.email, 'sessionCancelled', {
            clubName: req.school?.name || req.team.name,
...
router.get('/:teamId/schedule/:id/availability', authenticateToken, loadTeamAndClub, async (req, res, next) => {
  try {
    ...
    // Get all pupils on the team with their availability responses
    const result = await pool.query(
      `SELECT p.id AS pupil_id, p.name AS player_name,
        ar.id AS response_id, ar.response, ar.reason,
        ar.responded_at, ar.attended,
        u.name AS responded_by_name
      FROM pupils p
      LEFT JOIN availability_responses ar ON ar.pupil_id = p.id AND ar.session_id = $1
...
router.post('/:teamId/schedule/:id/availability', authenticateToken, loadTeamAndClub, async (req, res, next) => {
  try {
    const { teamId, id } = req.params
    const { pupil_id, response, reason } = req.body

    if (!pupil_id || !response) {
      return res.status(400).json({ error: 'pupil_id and response are required' })
    }
    ...
    // Verify pupil belongs to team
    const playerCheck = await pool.query(
      'SELECT id FROM pupils WHERE id = $1 AND team_id = $2',
      [pupil_id, teamId]
    )
    if (playerCheck.rows.length === 0) {
      return res.status(400).json({ error: 'Pupil not found on this team' })
    }

    // Upsert availability response
    const result = await pool.query(
      `INSERT INTO availability_responses (session_id, pupil_id, response, reason, responded_by, responded_at)
       VALUES ($1, $2, $3, $4, $5, NOW())
       ON CONFLICT (session_id, pupil_id)
       DO UPDATE SET response = $3, reason = $4, responded_by = $5, responded_at = NOW()
       RETURNING *`,
      [id, pupil_id, response, reason || null, req.user.id]
    )
...
router.put('/:teamId/schedule/:id/attendance', authenticateToken, loadTeamAndClub, async (req, res, next) => {
  try {
    ...
    // attendance should be an array of { pupil_id, attended }
    if (!Array.isArray(attendance)) {
      return res.status(400).json({ error: 'attendance must be an array of { pupil_id, attended }' })
    }
    ...
    for (const entry of attendance) {
      ...
      const result = await pool.query(
        `INSERT INTO availability_responses (session_id, pupil_id, response, responded_by, responded_at, attended)
         VALUES ($1, $2, 'available', $3, NOW(), $4)
         ON CONFLICT (session_id, pupil_id)
         DO UPDATE SET attended = $4
         RETURNING id, pupil_id, attended`,
        [id, entry.pupil_id, req.user.id, entry.attended]
      )
      if (result.rows.length > 0) {
...
router.get('/schools/:schoolId/attendance/stats', authenticateToken, loadSchool, requireSchoolFeature('attendance_tracking'), requireSchoolRole('owner', 'admin'), async (req, res, next) => {
  try {
    ...
    `
    const teamParams = [schoolId, dateFrom, dateTo]
    let teamParamIdx = 4
    ...
    // Top absentees (pupils with lowest attendance rates)
    const absenteesResult = await pool.query(
      `SELECT p.id AS pupil_id, p.name AS player_name, t.name AS team_name,
      ...
      JOIN availability_responses ar ON ar.pupil_id = p.id
    ...
    const responseRateResult = await pool.query(
      `SELECT
        COUNT(DISTINCT ss.id) AS total_sessions,
        (SELECT COUNT(DISTINCT p.id) FROM pupils p JOIN teams t ON p.team_id = t.id WHERE t.school_id = $1 AND p.is_active = true) AS total_players,
        COUNT(ar.id) AS total_responses,
