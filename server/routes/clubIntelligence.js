...
async function trackAIUsage(clubId, feature, model, inputTokens, outputTokens) {
  try {
    const costGBP = estimateCostGBP(inputTokens, outputTokens)
    await pool.query(
      `INSERT INTO ai_usage (club_id, feature, model, input_tokens, output_tokens, estimated_cost_gbp)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [clubId, feature, model, inputTokens, outputTokens, costGBP]
    )
    return costGBP
  } catch (error) {
...
async function checkMonthlyUsageCap(clubId, tier) {
...
// POST /:clubId/matches/:matchId/generate-report — Generate AI match report
router.post(
  '/:clubId/matches/:matchId/generate-report',
  authenticateToken,
  loadClub,
  requireClubRole('owner', 'admin', 'coach'),
  requireClubFeature('ai_match_reports'),
  async (req, res) => {
    try {
      ...
      // Check monthly cap
      const capCheck = await checkMonthlyUsageCap(clubId, club.subscription_tier)
      if (!capCheck.allowed) {
        return res.status(429).json({
          error: 'Monthly AI usage limit reached',
          used: capCheck.used,
          cap: capCheck.cap,
          message: `Your club has used £${capCheck.used} of your £${capCheck.cap} monthly AI budget. Upgrade your plan for more.`,
        })
      }
      ...
      const insertResult = await pool.query(
        `INSERT INTO match_reports (club_id, team_id, match_id, report_text, status, model_used, generation_cost_tokens)
         VALUES ($1, $2, $3, $4, 'draft', 'claude-sonnet-4-6', $5)
         RETURNING *`,
        [clubId, match.team_id, matchId, aiResult.text, totalTokens]
      )
...
// PUT /:clubId/matches/:matchId/report — Edit/approve report
router.put(
  '/:clubId/matches/:matchId/report',
  authenticateToken,
  loadClub,
  requireClubRole('owner', 'admin', 'coach'),
  async (req, res) => {
    try {
      ...
      values.push(matchId, clubId)
      const result = await pool.query(
        `UPDATE match_reports
         SET ${updates.join(', ')}
         WHERE match_id = $${idx++} AND club_id = $${idx++}
         RETURNING *`,
        values
      )
...
// POST /:clubId/matches/:matchId/report/send — Send report to parents
router.post(
  '/:clubId/matches/:matchId/report/send',
  authenticateToken,
  loadClub,
  requireClubRole('owner', 'admin', 'coach'),
  requireClubFeature('ai_match_reports'),
    ...
    try {
      ...
      const reportResult = await pool.query(
        `SELECT mr.*, m.opponent, t.name as team_name, t.id as team_id
         ...
         WHERE mr.match_id = $1 AND mr.club_id = $2
      ...
      // Get guardians linked to players in this team
      const guardiansResult = await pool.query(
        `SELECT DISTINCT g.email, g.first_name, g.last_name
         ...
         WHERE p.team_id = $1 AND g.club_id = $2 AND g.email IS NOT NULL`,
        [report.team_id, clubId]
      )
...
// GET /:clubId/insights — All insights (paginated)
router.get(
  '/:clubId/insights',
  authenticateToken,
  loadClub,
  requireClubRole('owner', 'admin', 'coach', 'secretary'),
  async (req, res) => {
    try {
      ...
      const [insightsResult, countResult] = await Promise.all([
        ...
        pool.query(
          `SELECT COUNT(*) FROM ai_insights WHERE club_id = $1`,
          [clubId]
        ),
      ])
...
// GET /:clubId/teams/:teamId/insights — Team-specific insights
router.get(
  '/:clubId/teams/:teamId/insights',
  authenticateToken,
  loadClub,
  requireClubRole('owner', 'admin', 'coach'),
  async (req, res) => {
    try {
      ...
      const [insightsResult, countResult] = await Promise.all([
        pool.query(
          `SELECT * FROM ai_insights
           WHERE club_id = $1 AND (team_id = $2 OR team_id IS NULL)
           ORDER BY created_at DESC
           LIMIT $3 OFFSET $4`,
          [clubId, teamId, limit, offset]
        ),
        pool.query(
...
// PUT /:clubId/insights/:id — Mark insight as seen/actioned
router.put(
  '/:clubId/insights/:id',
  authenticateToken,
  loadClub,
    ...
    try {
      ...
      const updateResult = await pool.query(
        `UPDATE ai_insights
         ...
         RETURNING *`,
        [JSON.stringify([userId]), actioned, id, clubId]
      )
...
// POST /:clubId/reports/season-summary — Generate season report
router.post(
  '/:clubId/reports/season-summary',
  authenticateToken,
  loadClub,
  requireClubRole('owner', 'admin'),
  requireClubFeature('ai_season_report'),
  async (req, res) => {
    try {
      const { clubId } = req.params
      const club = req.club
      ...
      // Gather club data for the report
      // Membership data
      const membershipResult = await pool.query(
        `SELECT
         ...
         LEFT JOIN guardians g ON g.club_id = $1
         LEFT JOIN club_members cm ON cm.club_id = $1
         WHERE t.club_id = $1`,
        [clubId]
      )
      ...
      try {
        const finResult = await pool.query(
          `SELECT
           ...
           WHERE club_id = $1 AND created_at >= NOW() - INTERVAL '12 months'`,
          [clubId]
        )
      ...
      // Store as an insight
      await pool.query(
        `INSERT INTO ai_insights (club_id, insight_type, title, content, data_snapshot, priority, model_used)
         VALUES ($1, 'season_summary', 'Season Summary Report', $2, $3, 'normal', 'claude-sonnet-4-6')`,
        [clubId, JSON.stringify(aiResult.text), JSON.stringify({
...
// GET /:clubId/reports/season-summary/latest — Get latest report
router.get(
  '/:clubId/reports/season-summary/latest',
  authenticateToken,
  loadClub,
  requireClubRole('owner', 'admin', 'coach', 'secretary'),
  async (req, res) => {
    try {
      ...
      const result = await pool.query(
        `SELECT * FROM ai_insights
         WHERE club_id = $1 AND insight_type = 'season_summary'
...
// POST /:clubId/grants/draft — Generate grant draft
router.post(
  '/:clubId/grants/draft',
  authenticateToken,
  loadClub,
  requireClubRole('owner', 'admin', 'secretary'),
  requireClubFeature('ai_grant_helper'),
  async (req, res) => {
    try {
      const { clubId } = req.params
      const club = req.club
      ...
      // Gather club stats for the application
      ...
      const aiResult = await generateGrantDraft({
        clubName: club.name,
        location: club.city || club.county || club.postcode,
        faAffiliation: club.fa_affiliation_number,
        charterStandard: club.charter_standard,
      ...
      const draftResult = await pool.query(
        `INSERT INTO grant_drafts (club_id, grant_type, description, estimated_cost, draft_text, created_by)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING *`,
        [clubId, grant_type, description, estimated_cost || null, aiResult.text, req.user.id]
      )
...
// GET /:clubId/grants/drafts — List saved drafts
router.get(
  '/:clubId/grants/drafts',
  authenticateToken,
  loadClub,
  requireClubRole('owner', 'admin', 'secretary'),
  async (req, res) => {
...
// PUT /:clubId/grants/drafts/:id — Update draft
router.put(
  '/:clubId/grants/drafts/:id',
  authenticateToken,
  loadClub,
  requireClubRole('owner', 'admin', 'secretary'),
  async (req, res) => {
...
// POST /:clubId/compliance/ai-analysis — Run AI gap analysis
router.post(
  '/:clubId/compliance/ai-analysis',
  authenticateToken,
  loadClub,
  requireClubRole('owner', 'admin'),
  requireClubFeature('ai_compliance_analysis'),
  async (req, res) => {
    try {
      const { clubId } = req.params
      const club = req.club
        ...
        `SELECT t.id, t.name
         ...
         WHERE t.club_id = $1
           AND t.id NOT IN (
             ...
             JOIN compliance_records cr ON cr.user_id = tm.user_id AND cr.club_id = $1
...
// GET /:clubId/compliance/ai-analysis/latest — Most recent analysis
router.get(
  '/:clubId/compliance/ai-analysis/latest',
  authenticateToken,
  loadClub,
  requireClubRole('owner', 'admin'),
  async (req, res) => {
    try {
      ...
      const result = await pool.query(
        `SELECT * FROM ai_insights
         WHERE club_id = $1 AND insight_type = 'compliance_analysis'
         ORDER BY created_at DESC LIMIT 1`,
        [clubId]
      )
...
// GET /:clubId/coaches/:userId/development-suggestions — AI suggestions
router.get(
  '/:clubId/coaches/:userId/development-suggestions',
  authenticateToken,
  loadClub,
  requireClubRole('owner', 'admin', 'coach'),
  requireClubFeature('ai_coach_development'),
  async (req, res) => {
    try {
      ...
      const coachResult = await pool.query(
        `SELECT u.name, cr.coaching_badges
         FROM users u
         LEFT JOIN compliance_records cr ON cr.user_id = u.id AND cr.club_id = $1
         WHERE u.id = $2`,
        [clubId, userId]
      )
      ...
      const [sessionResult, videoResult, observationResult, themesResult] = await Promise.all([
        pool.query(
          `SELECT COUNT(*) as count FROM training_sessions ts
           ...
           WHERE t.club_id = $1 AND ts.date >= NOW() - INTERVAL '12 months'`,
          [clubId]
        ),
        pool.query(
          `SELECT COUNT(*) as count FROM video_ai_analysis va
           ...
           WHERE t.club_id = $1 AND va.created_at >= NOW() - INTERVAL '12 months'`,
          [clubId]
        ),
        pool.query(
        ...
        pool.query(
          `SELECT ts.focus_areas as theme, COUNT(*) as count
           ...
           JOIN teams t ON t.id = ts.team_id
           WHERE t.club_id = $1 AND ts.focus_areas IS NOT NULL AND ts.date >= NOW() - INTERVAL '6 months'
...
// =============================================
// AI USAGE STATS (for club dashboard)
// =============================================

// GET /:clubId/ai-usage — Get current month's AI usage
router.get(
  '/:clubId/ai-usage',
  authenticateToken,
  loadClub,
  ...
  async (req, res) => {
    try {
      ...
      const cap = TIER_MONTHLY_CAPS[club.subscription_tier] || 999
...
// POST /:clubId/teams/:teamId/attendance-insights — Generate AI attendance insights
router.post(
  '/:clubId/teams/:teamId/attendance-insights',
  authenticateToken,
  loadClub,
  requireClubRole('owner', 'admin', 'coach'),
  requireClubFeature('ai_attendance_insights'),
  async (req, res) => {
    try {
      const { clubId, teamId } = req.params
      const club = req.club
      ...
      const teamResult = await pool.query(
        'SELECT name, age_group FROM teams WHERE id = $1 AND club_id = $2',
        [teamId, clubId]
      )
      ...
      const [sessionsResult, playerPatternsResult] = await Promise.all([
        pool.query(
          `SELECT
           ...
           LEFT JOIN training_attendance ta ON ta.session_id = ts.id AND ta.player_id = p.id
           WHERE ts.team_id = $1
             AND p.team_id = $1
      ...
      await pool.query(
        `INSERT INTO ai_insights (club_id, team_id, insight_type, title, content, data_snapshot, priority)
