...
router.get('/:clubId/events', authenticateToken, loadClub, requireClubFeature('events'), requireClubRole('owner', 'admin', 'secretary', 'coach', 'treasurer'), async (req, res, next) => {
  try {
...
// Create a new event
router.post('/:clubId/events', authenticateToken, loadClub, requireClubFeature('events'), requireClubRole('owner', 'admin'), async (req, res, next) => {
  try {
    const { clubId } = req.params
    const {
    ...
    if (price && price > 0 && stripe && req.club.stripe_account_id) {
      try {
        const product = await stripe.products.create({
          name: title,
          description: description || `Event: ${title}`,
          metadata: { club_id: clubId, event_type },
        }, {
          stripeAccount: req.club.stripe_account_id,
        })
    ...
    const result = await pool.query(
      `INSERT INTO club_events (
        club_id, title, description, event_type, start_date, end_date,
        venue_name, venue_address, venue_postcode,
        target_audience, target_team_ids, target_age_groups,
        allow_external, max_participants, waitlist_enabled,
        price, early_bird_price, early_bird_deadline, sibling_discount_percent,
      ...
      [
        clubId, title, description || null, event_type, start_date, end_date || null,
...
router.get('/:clubId/events/:id', authenticateToken, loadClub, requireClubFeature('events'), requireClubRole('owner', 'admin', 'secretary', 'coach', 'treasurer'), async (req, res, next) => {
  try {
    ...
    const result = await pool.query(
      ...
      FROM club_events ce
      WHERE ce.id = $1 AND ce.club_id = $2`,
      [id, clubId]
    )
...
// Update event
router.put('/:clubId/events/:id', authenticateToken, loadClub, requireClubFeature('events'), requireClubRole('owner', 'admin'), async (req, res, next) => {
  try {
    const { clubId, id } = req.params
    const {
      title, description, event_type, start_date, end_date,
...
router.delete('/:clubId/events/:id', authenticateToken, loadClub, requireClubFeature('events'), requireClubRole('owner', 'admin'), async (req, res, next) => {
  try {
    const { clubId, id } = req.params

    const result = await pool.query(
      `UPDATE club_events SET status = 'cancelled', updated_at = NOW()
...
router.get('/:clubId/events/:id/registrations', authenticateToken, loadClub, requireClubFeature('events'), requireClubRole('owner', 'admin', 'secretary'), async (req, res, next) => {
  try {
    const { clubId, id } = req.params
    ...
    const result = await pool.query(
      ...
      FROM event_registrations er
      LEFT JOIN players p ON er.player_id = p.id
      LEFT JOIN guardians g ON er.guardian_id = g.id
...
router.get('/:clubId/events/:id/export', authenticateToken, loadClub, requireClubFeature('events'), requireClubRole('owner', 'admin', 'secretary'), async (req, res, next) => {
  try {
    const { clubId, id } = req.params

    // Verify event belongs to club
    const eventResult = await pool.query(
      'SELECT title FROM club_events WHERE id = $1 AND club_id = $2',
      [id, clubId]
    )
...
router.put('/:clubId/events/:id/attendance', authenticateToken, loadClub, requireClubFeature('events'), requireClubRole('owner', 'admin', 'coach'), async (req, res, next) => {
  try {
    const { clubId, id } = req.params
    ...
    const eventCheck = await pool.query(
      'SELECT id FROM club_events WHERE id = $1 AND club_id = $2',
      [id, clubId]
    )
    ...
    for (const entry of attendance) {
      const result = await pool.query(
        `UPDATE event_registrations SET attended = $1
         WHERE id = $2 AND event_id = $3 AND club_id = $4
         RETURNING id, attended`,
        [entry.attended, entry.registration_id, id, clubId]
      )
      if (result.rows.length > 0) {
...
router.post('/public/:eventId/register', async (req, res, next) => {
  try {
    ...
    if (!player_id && !external_name) {
      return res.status(400).json({ error: 'Either player_id or external_name is required' })
    }

    if (!player_id && !external_email) {
    ...
    if (!currentPrice || currentPrice <= 0) {
      ...
      await pool.query(
        'UPDATE club_events SET current_participants = current_participants + 1 WHERE id = $1',
        [eventId]
      )
    ...
    // Create Stripe Checkout Session if Stripe is configured and club has connected account
    if (stripe && event.stripe_account_id) {
      try {
        ...
        return res.status(201).json({
          registration,
          checkout_url: null,
          stripe_error: 'Payment processing unavailable. Please contact the club.',
          waitlisted: false,
        })
      }
    }
