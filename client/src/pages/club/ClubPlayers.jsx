...
export default function ClubPlayers() {
  ...
  async function loadPlayers() {
    try {
      ...
      const res = await clubService.getPlayers(club.id, params)
      setPlayers(res.data)
    } catch (err) {
  ...
  return (
    <div className="p-4 sm:p-6 max-w-6xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-navy-400 text-sm mt-1">{players.length} player{players.length !== 1 ? 's' : ''} across all teams</p>
      </div>
      ...
      {players.length === 0 ? (
        <div className="bg-navy-900 border border-navy-800 rounded-xl p-8 text-center">
          ...
          <p className="text-navy-400 text-sm">Players will appear here when they're registered with the club.</p>
        </div>
      ) : (
