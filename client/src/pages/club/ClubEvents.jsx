...
export default function ClubEvents() {
  ...
  async function loadData() {
    try {
      const [eventsRes, teamsRes] = await Promise.all([
        clubEventsService.getEvents(club.id),
  ...
  async function loadRegistrations(eventId) {
    ...
    try {
      const res = await clubEventsService.getRegistrations(club.id, eventId)
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (editingEvent) {
        await clubEventsService.updateEvent(club.id, editingEvent.id, payload)
        toast.success('Event updated')
      } else {
        await clubEventsService.createEvent(club.id, payload)
  ...
  async function handleCancel(eventId) {
    ...
    try {
      await clubEventsService.cancelEvent(club.id, eventId)
  ...
  async function handleExport(eventId) {
    try {
      const res = await clubEventsService.exportRegistrations(club.id, eventId)
  ...
  async function handleAttendanceToggle(eventId, registrationId, attended) {
    try {
      await clubEventsService.updateAttendance(club.id, eventId, {
    ...
    <div className="p-4 sm:p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-navy-400 text-sm mt-1">Manage camps, tournaments and club events</p>
        </div>
        {canManage && (
          <button
            onClick={() => {
