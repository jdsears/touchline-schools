...
export default function ClubLayout() {
  ...
  async function loadClub() {
    setLoadError(null)
    try {
      // First get public info, then try to get full club via my clubs
      ...
      <div className="min-h-screen bg-navy-950 flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-xl font-bold text-white mb-2">
            {loadError === 'error' ? 'Failed to load club' : 'Club not found'}
          </h2>
          <p className="text-navy-400 mb-4">
            {loadError === 'error'
              ? 'Something went wrong loading the club. Please check your connection and try again.'
      ...
      <div className="lg:pl-64">
        ...
        <header className="sticky top-0 z-30 lg:hidden flex items-center justify-between h-16 px-4 bg-navy-900/80 backdrop-blur-md border-b border-navy-800">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-navy-50 text-sm truncate max-w-[200px]">{club.name}</span>
          </div>
        ...
        <main className="min-h-[calc(100vh-4rem)] lg:min-h-screen">
          <ErrorBoundary key={location.pathname}>
            <Suspense fallback={<div className="flex items-center justify-center min-h-[50vh]"><div className="spinner w-8 h-8" /></div>}>
              <Outlet context={{ club, myRole, refreshClub: loadClub }} />
            </Suspense>
          </ErrorBoundary>
        </main>
      </div>
