...
export default function ClubInsights() {
  ...
  useEffect(() => {
    if (club?.id) loadInsights()
  }, [club?.id])

  async function loadInsights() {
    try {
      const res = await clubIntelligenceService.getInsights(club.id)
  ...
  async function handleMarkActioned(insightId) {
    ...
    try {
      await clubIntelligenceService.markInsight(club.id, insightId, { status: 'actioned' })
      setInsights(prev =>
  ...
  async function handleDismiss(insightId) {
    setActioningId(insightId)
    try {
      await clubIntelligenceService.markInsight(club.id, insightId, { status: 'dismissed' })
      setInsights(prev =>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-navy-400 text-sm mt-0.5">
              Intelligent observations about your club
              {unseenCount > 0 && (
      ...
      {filteredInsights.length === 0 ? (
        <div className="bg-navy-900 border border-navy-800 rounded-xl p-8 text-center">
          ...
          <p className="text-navy-400 text-sm max-w-md mx-auto">
            Insights are automatically created as your club data grows. Keep adding matches, tracking attendance, and managing your club to see AI-powered observations here.
          </p>
        </div>
      ) : (
        <div className="space-y-4">
