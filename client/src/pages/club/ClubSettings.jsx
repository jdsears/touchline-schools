...
export default function ClubSettings() {
  const { club, myRole, refreshClub } = useOutletContext()
  ...
  useEffect(() => {
    if (club?.id && !charityLoaded) {
      giftAidService.getCharitySettings(club.id).then(res => {
  ...
  async function handleSaveCharity(e) {
    ...
    try {
      const res = await giftAidService.updateCharitySettings(club.id, charityForm)
  ...
  useEffect(() => {
    if (club) {
      setForm({
        ...
        fa_affiliation_number: club.fa_affiliation_number || '',
        league: club.league || '',
        charter_standard: club.charter_standard || '',
        primary_color: club.primary_color || '#1a365d',
        secondary_color: club.secondary_color || '#38a169',
        season_start_month: club.season_start_month || 9,
        season_end_month: club.season_end_month || 6,
      })
    }
  }, [club])
  ...
  async function handleSave(e) {
    ...
    setSaving(true)
    try {
      await clubService.updateClub(club.id, form)
  ...
  return (
    <div className="p-4 sm:p-6 max-w-3xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-navy-400 text-sm mt-1">Manage your club details and branding</p>
      </div>
        ...
        <form onSubmit={handleSaveCharity} className="space-y-6 mt-8">
          <section className="bg-navy-900 border border-navy-800 rounded-xl p-5 space-y-4">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-navy-800 border border-navy-700 rounded-lg px-3 py-2 text-white text-sm focus:ring-2 focus:ring-pitch-600 focus:border-transparent"
                  placeholder="Community Amateur Sports Club number"
                />
              </div>
              <div>
            ...
            <div className="flex items-center justify-between pt-2 border-t border-navy-800">
              <div>
                <label className="text-sm text-white font-medium">Enable Gift Aid for this club</label>
