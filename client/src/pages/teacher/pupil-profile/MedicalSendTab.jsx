  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {!m ? (
          ...
          <div className="space-y-3 text-sm">
            ...
            {m.last_reviewed_date && (
              <div className="text-xs text-tertiary flex items-center gap-1 pt-2">
      ...
      <div className="bg-card rounded-xl border border-border-default p-5">
        ...
        {!s ? (
          <p className="text-sm text-tertiary text-center py-6">No SEND profile recorded.</p>
        ) : (
          <div className="space-y-3 text-sm">
            <div className="flex items-center gap-2">
              <span className={`px-2 py-0.5 rounded text-xs font-medium ${s.ehcp_status ? 'bg-sky-500/20 text-sky-400' : 'bg-border-default text-secondary'}`}>
                {s.ehcp_status ? 'EHCP in place' : 'No EHCP'}
              </span>
              {s.ehcp_number && <span className="text-xs text-tertiary font-mono">{s.ehcp_number}</span>}
            </div>
...
function Row({ label, value }) {
  ...
  return (
    <div>
      <div className="text-xs text-tertiary uppercase tracking-wide mb-0.5">{label}</div>
