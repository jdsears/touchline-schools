...
export default function TeacherClassDetail() {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <Link
        to="/teacher/classes"
        className="inline-flex items-center gap-1 text-secondary hover:text-link text-sm mb-6 transition-colors"
      >
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mt-1">
              ...
              <span className="text-sm text-secondary">{group.key_stage}</span>
              <span className="text-tertiary">|</span>
              <span className="text-sm text-secondary">{group.academic_year}</span>
            </div>
          </div>
        </div>
        <button
          ...
          {group.units && group.units.length > 0 ? (
            <div className="space-y-3">
              {group.units.map(unit => (
                <div key={unit.id} className="bg-card rounded-xl border border-border-default p-5">
                  <div className="flex items-center justify-between">
                    <div>
                ...
                {group.pupils.map(pupil => (
                  <div key={pupil.id} className="flex items-center justify-between py-2 px-3 rounded-lg hover:bg-subtle group">
                    <Link to={`/teacher/hod/pupils/${pupil.id}`} className="flex-1 min-w-0">
                      ...
                      {pupil.house && (
                        <span className="ml-2 text-xs text-tertiary">{pupil.house}</span>
                      )}
                    </Link>
                    <button
      ...
      {showAddPupils && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-card rounded-xl border border-border-strong w-full max-w-md mx-4 p-6 max-h-[80vh] flex flex-col">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-primary">Add Pupils to {group.name}</h2>
              <button onClick={() => setShowAddPupils(false)} className="text-secondary hover:text-link">
                <X className="w-5 h-5" />
              </button>
            </div>
              ...
              <div className="flex-1 overflow-y-auto space-y-1 min-h-0">
                {(() => {
                  ...
                  return filtered.map(p => {
                    ...
                    return (
                      <div key={p.id} className="flex items-center justify-between py-2 px-3 rounded-lg hover:bg-subtle">
                        <div>
                          <span className="text-sm text-primary">{displayName}</span>
                          <div className="flex items-center gap-2 mt-0.5">
                            {p.year_group && <span className="text-xs text-tertiary">Year {p.year_group}</span>}
                            {p.house && <span className="text-xs text-tertiary">{p.house}</span>}
                          </div>
                        </div>
                        <button
            ...
            <div className="flex justify-end pt-4 border-t border-border-default mt-4">
              <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-card rounded-xl border border-border-strong w-full max-w-md mx-4 p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-semibold text-primary">Add Sport Unit</h2>
              <button onClick={() => setShowAddUnit(false)} className="text-secondary hover:text-link">
                <X className="w-5 h-5" />
              </button>
            </div>
            <form onSubmit={handleAddUnit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm text-secondary mb-1">Unit Name</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm text-secondary mb-1">Lessons (optional)</label>
                  <input
                    type="number"
                    placeholder="e.g., 6"
