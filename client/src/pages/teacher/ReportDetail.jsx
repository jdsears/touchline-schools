...
export default function ReportDetail() {
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <button onClick={() => navigate(`/teacher/hod/reporting/windows/${windowId}`)} className="flex items-center gap-1.5 text-navy-400 hover:text-white text-sm mb-5 transition-colors">
      ...
      <div className="card p-5 mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-white">{report.first_name} {report.last_name}</h1>
      ...
      {/* Assessment summary */}
      <div className="card p-5 mb-4">
        <h2 className="text-sm font-semibold text-navy-400 uppercase tracking-wide mb-3 flex items-center gap-2">
          <BookOpen className="w-4 h-4" /> Assessment
        </h2>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <p className="text-xs text-navy-500 mb-1">Sport / Unit</p>
            <p className="text-white text-sm">{report.unit_name || report.unit_sport || report.sport || '—'}</p>
          </div>
          <div />
          <div>
            <p className="text-xs text-navy-500 mb-1">Attainment</p>
            ...
            ) : <span className="text-navy-500 text-sm">Not set</span>}
          </div>
          <div>
            <p className="text-xs text-navy-500 mb-1">Effort</p>
            {report.effort_grade ? (
      ...
      <div className="card p-5 mb-4">
        ...
        {report.teacher_comment ? (
          <p className="text-white text-sm leading-relaxed whitespace-pre-wrap">{report.teacher_comment}</p>
        ) : (
          <p className="text-navy-500 text-sm italic">No comment written yet.</p>
        )}
      </div>
      ...
      <div className="card p-5">
        ...
        <div className="space-y-2 text-sm">
          <div className="flex justify-between">
            <span className="text-navy-400">Written by</span>
            <span className="text-white">{report.teacher_name || '—'}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-navy-400">Last modified</span>
            <span className="text-white">{report.updated_at ? new Date(report.updated_at).toLocaleString('en-GB') : '—'}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-navy-400">Created</span>
            <span className="text-white">{report.created_at ? new Date(report.created_at).toLocaleString('en-GB') : '—'}</span>
          </div>
        </div>
      </div>
    </div>
