...
export default function TeacherCurriculum() {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="mb-8">
        <h1 className="font-display text-3xl font-bold tracking-[-0.015em] text-primary">Curriculum Overview</h1>
        <p className="text-secondary mt-1">National Curriculum PE areas and assessment strands</p>
      </div>
      ...
      {ks.primaryOnly && (
        <div className="flex items-start gap-3 bg-subtle/60 border border-border-strong rounded-xl p-4 mb-6">
          <AlertCircle className="w-4 h-4 text-secondary shrink-0 mt-0.5" />
          <p className="text-sm text-secondary">
            <strong className="text-secondary">{ks.label} covers a primary school phase.</strong> Your school may not teach these year groups.
      ...
      {/* KS description */}
      <p className="text-sm text-secondary mb-5">{ks.description}</p>
          ...
          <div key={area.name} className="bg-card rounded-xl border border-border-default p-5">
            ...
            <div className="flex flex-wrap gap-1">
              {area.sports.map(sport => (
                <span key={sport} className="px-2 py-0.5 bg-subtle rounded text-xs text-secondary">{sport}</span>
              ))}
            </div>
          </div>
        ))}
