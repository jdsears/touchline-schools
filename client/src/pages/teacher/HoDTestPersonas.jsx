    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          ...
          disabled={loading}
          className="flex items-center gap-2 px-3 py-2 bg-subtle hover:bg-border-default text-secondary rounded-lg text-sm transition-colors disabled:opacity-50"
        >
      ...
      {!loading && !error && personas.length === 0 && (
        <div className="text-center py-16">
          ...
          <code className="text-xs text-tertiary mt-2 block">
          ...
          {personas.map(p => (
            ...
            >
              ...
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className={`px-2 py-0.5 rounded text-xs font-medium ${YEAR_COLORS[p.year_group] || 'bg-border-default text-secondary'}`}>
                        Year {p.year_group}
                      </span>
                      <span className={`px-2 py-0.5 rounded text-xs font-medium ${HOUSE_COLORS[p.house] || 'bg-border-default text-secondary'}`}>
                        {p.house} House
                      </span>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Link
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-subtle hover:bg-border-default text-primary rounded-lg text-sm transition-colors"
                  >
              ...
              <div className="flex flex-wrap gap-4 pl-13">
                ...
                {p.sports && (
                  <div className="flex items-center gap-1.5 text-xs">
                    ...
                    <span className="text-tertiary">Sports</span>
                    <span className="text-secondary font-medium capitalize">{p.sports}</span>
                  </div>
                )}
              </div>
              ...
              <div className="mt-3 pl-13">
                <span className="text-xs text-tertiary">{p.email}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Info footer */}
      <div className="mt-8 bg-card/50 rounded-xl border border-border-subtle p-4">
        <h3 className="text-sm font-medium text-secondary mb-2 flex items-center gap-2">
