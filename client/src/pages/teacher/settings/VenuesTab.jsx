    ...
    <div className="space-y-4">
      ...
      {viewing && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setViewing(null)}>
          <div className="bg-card border border-border-default rounded-xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-primary">{viewing.name}</h3>
              <button onClick={() => setViewing(null)} className="text-secondary hover:text-link"><X className="w-5 h-5" /></button>
            </div>
            {viewing.address && <p className="text-sm text-secondary mb-1">{viewing.address}</p>}
            {viewing.postcode && <p className="text-sm text-secondary mb-3">{viewing.postcode}</p>}
            <Suspense fallback={<div className="bg-subtle rounded-lg h-60 flex items-center justify-center"><Loader2 className="w-5 h-5 animate-spin text-secondary" /></div>}>
            ...
            {viewing.contact_name && (
              <p className="text-xs text-secondary mt-3">Contact: {viewing.contact_name}{viewing.contact_phone ? ` - ${viewing.contact_phone}` : ''}</p>
            )}
          </div>
        </div>
      )}
      ...
      {editing && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setEditing(null)}>
          <div className="bg-card border border-border-default rounded-xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-primary">{editing === 'new' ? 'Add Venue' : 'Edit Venue'}</h3>
              <button onClick={() => setEditing(null)} className="text-secondary hover:text-link"><X className="w-5 h-5" /></button>
            </div>
...
function Detail({ label, text }) {
  return (
    <div className="mt-3">
      <p className="text-xs font-medium text-tertiary uppercase tracking-wider mb-0.5">{label}</p>
