  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {statusIcon && <div className="flex items-center gap-2 text-sm text-navy-300">{statusIcon}{config.lastSyncStatus === 'success' ? 'Connected' : config.lastSyncStatus === 'error' ? 'Error' : 'Not synced'}</div>}
      </div>
      ...
      <div className="bg-navy-900 border border-navy-800 rounded-xl p-5 space-y-4">
        <h3 className="text-sm font-semibold text-white">iSAMS Configuration</h3>
        ...
        <div className="grid grid-cols-2 gap-4">
          <label className="block text-xs text-navy-400">
            ...
            <select value={form.syncFrequency} onChange={e => setForm(f => ({ ...f, syncFrequency: e.target.value }))}
              className="block w-full mt-1 bg-navy-800 border border-navy-700 rounded-lg px-3 py-2 text-sm text-white">
      ...
      {config?.provider && (
        <div className="bg-navy-900 border border-navy-800 rounded-xl p-5">
          <h3 className="text-sm font-semibold text-white mb-3">Sync Controls</h3>
          <div className="flex gap-2">
            <button onClick={() => handleSync(true)} disabled={syncing}
              className="flex items-center gap-1.5 px-3 py-2 bg-navy-800 hover:bg-navy-700 text-navy-300 rounded-lg text-sm border border-navy-700">
            ...
            <button onClick={() => handleSync(false)} disabled={syncing}
              className="flex items-center gap-1.5 px-3 py-2 bg-pitch-600 hover:bg-pitch-500 disabled:opacity-50 text-white rounded-lg text-sm">
              {syncing ? <Loader2 className="w-4 h-4 animate-spin" /> : <Play className="w-4 h-4" />} Sync Now
            </button>
          </div>
          {config.lastSyncAt && <p className="text-xs text-navy-500 mt-2">Last sync: {new Date(config.lastSyncAt).toLocaleString('en-GB')}</p>}
        </div>
      )}
      ...
      {history.length > 0 && (
        <div className="bg-navy-900 border border-navy-800 rounded-xl p-5">
          <h3 className="text-sm font-semibold text-white mb-3">Recent Sync History</h3>
          <div className="space-y-2">
            {history.map(h => (
              <div key={h.id} className="flex items-center gap-3 text-sm py-2 border-b border-navy-800/50 last:border-0">
                ...
                <div className="flex-1">
                  <span className="text-navy-300">{new Date(h.started_at).toLocaleString('en-GB')}</span>
                  {h.is_dry_run && <span className="ml-2 text-[10px] bg-navy-700 text-navy-400 px-1.5 py-0.5 rounded">Dry run</span>}
                </div>
                {h.summary && <span className="text-xs text-navy-400">
  ...
  return (
    <label className="block text-xs text-navy-400">
      ...
      <input type={type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder}
        className="block w-full mt-1 bg-navy-800 border border-navy-700 rounded-lg px-3 py-2 text-sm text-white placeholder-navy-500" />
    </label>
  )
}
