...
export default function ClubSafeguardingIncidents() {
  ...
  useEffect(() => {
    if (school?.id && hasAccess) loadIncidents()
  ...
  async function loadIncidentDetail(incidentId) {
    try {
      const res = await clubSafeguardingService.getIncident(school.id, incidentId)
  ...
  async function handleAddAction(incidentId) {
    ...
    try {
      ...
      await clubSafeguardingService.updateIncident(school.id, incidentId, {
