    ...
    <div className="p-4 sm:p-6 max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-secondary text-sm mt-1">{teams.length} team{teams.length !== 1 ? 's' : ''}</p>
        </div>
        {canManage && (
          <button
      ...
      {showAdd && (
        <form onSubmit={handleCreate} className="bg-card border border-border-default rounded-xl p-4 space-y-4">
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
              <label className="block text-xs text-secondary mb-1">Team Name *</label>
              <input
            ...
            <div>
              <label className="block text-xs text-secondary mb-1">Age Group</label>
              <select
      ...
      {teams.length === 0 ? (
        <div className="bg-card border border-border-default rounded-xl p-8 text-center">
          <Building2 className="w-12 h-12 text-tertiary mx-auto mb-3" />
          <h3 className="text-lg font-medium text-primary mb-1">No teams yet</h3>
          <p className="text-secondary text-sm">Create your first team to get started.</p>
        </div>
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {teams.map((team) => (
            <div key={team.id} className="bg-card border border-border-default rounded-xl p-5 hover:border-border-strong transition-colors">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <h3 className="font-semibold text-primary">{team.name}</h3>
                  <div className="flex items-center gap-2 mt-1">
                    {team.age_group && (
                      <span className="text-xs bg-subtle text-secondary px-2 py-0.5 rounded-full">{team.age_group}</span>
                    )}
                    {team.team_type && (
                      <span className="text-xs bg-subtle text-secondary px-2 py-0.5 rounded-full capitalize">{team.team_type}</span>
                    )}
                  </div>
                </div>
