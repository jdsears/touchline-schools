...
const SAFEGUARDING_ROLES = [
  ...
  },
  {
    ...
    description: 'Senior person responsible for safeguarding strategy and escalation. Often the school chair or secretary.',
    required: false,
  },
]
...
export default function ClubSafeguardingRoles() {
  ...
  async function loadData() {
    try {
      const [rolesRes, membersRes] = await Promise.all([
        clubSafeguardingService.getRoles(school.id),
        clubService.getMembers(school.id),
      ])
  ...
  async function handleAssign(roleKey) {
    ...
    try {
      await clubSafeguardingService.assignRole(school.id, {
  ...
  async function handleRemove(roleId, roleName, memberName) {
    ...
    try {
      await clubSafeguardingService.removeRole(school.id, roleId)
