...
export default function PlayerDetail() {
  ...
  const { updatePlayer, deletePlayer, players: teamPlayers } = useTeam()
  ...
  async function handleSave() {
    ...
    if (result.success) {
      ...
      setPlayer(result.player || editData)
      setEditData(result.player || editData)
      setEditing(false)
  ...
  async function handleAwardBadge() {
    ...
    try {
      ...
      toast.success('Badge awarded! Player will be notified.')
    } catch (error) {
  ...
  async function handleAnalyzeAttributes() {
    ...
    try {
      ...
      // Immediately update player with extracted attributes from the response
      ...
      try {
        ...
        console.error('Player refresh after analysis failed (non-fatal):', refreshErr)
      }
  ...
  async function handleGenerateIdp() {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/players/${id}/idp/generate`, {
        method: 'POST',
        headers: {
  ...
  if (!player) {
    return (
      <div className="p-6 lg:p-8 max-w-4xl mx-auto">
        <Link to="/players" className="inline-flex items-center gap-2 text-navy-400 hover:text-white mb-6">
          <ArrowLeft className="w-4 h-4" />
          Back to Players
        </Link>
        <div className="card p-8 text-center">
          <User className="w-12 h-12 text-navy-600 mx-auto mb-4" />
          <h1 className="font-display text-xl font-bold text-white mb-2">Player Not Found</h1>
          <p className="text-navy-400">This player may have been deleted or doesn't exist.</p>
        </div>
      </div>
    )
  }
            ...
            <div className="flex items-start justify-between gap-4 mb-3">
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {getSortedPositions(player.positions).map(({ position: pos, priority }) => (
                    <span
                      key={pos}
      ...
      <AnimatePresence mode="wait">
        {activeTab === 'overview' && (
          ...
          >
            ...
            {player.parent_contact && parseParentContacts(player.parent_contact).length > 0 && (
              <div className="card p-6">
                ...
                <div className="grid gap-3">
                  {parseParentContacts(player.parent_contact).map((contact, index) => (
                    <div
                      key={index}
            ...
            {player.notes && (
              <div className="card p-6">
                ...
                <p className="text-navy-300 whitespace-pre-wrap">{player.notes}</p>
              </div>
            )}

            {/* Discreet Notes - Private coach notes about player psychology/communication */}
            {player.discreet_notes && (
              <div className="card p-6 border border-navy-600 bg-navy-900/50">
                ...
                <p className="text-xs text-navy-500 mb-3">
                  Private notes about player psychology and communication. Never shared with AI or the player/parent portal.
                </p>
                <p className="text-navy-300 whitespace-pre-wrap">{player.discreet_notes}</p>
              </div>
            )}
            ...
            <PlayerRadarChart player={player} teamPlayers={teamPlayers} />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              >
                ...
                <div className="text-left">
                  <p className="font-medium text-white">Player AI Coach</p>
                  <p className="text-sm text-navy-400">Personal assistant for {player.name?.split(' ')[0]}</p>
                </div>
                <ChevronRight className="w-5 h-5 text-navy-600 ml-auto" />
              </Link>

              <Link
                to={`/chat?q=Give me training recommendations for ${player.name}`}
                className="card-hover p-4 flex items-center gap-3"
              >
            ...
            {idp ? (
              ...
              <div className="card p-8 text-center">
                ...
                <p className="text-navy-400 mb-4 max-w-md mx-auto">
                  Generate an AI-powered Individual Development Plan based on observations and player data.
                  {observations.length === 0 && (
          ...
          >
            ...
            >
              <div className="p-6 border-b border-navy-800 flex items-center justify-between">
                <h2 className="font-display text-xl font-semibold text-white">Edit Player</h2>
                <button
                  onClick={() => setEditing(false)}
              ...
              <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                ...
                <div className="p-4 bg-navy-800/50 rounded-lg border border-navy-600">
                  ...
                  <p className="text-xs text-navy-500 mb-2">
                    Private notes about player psychology, communication style, or sensitive context. These notes are never shared with the AI assistant or visible to players/parents.
                  </p>
                  <textarea
              ...
              <form onSubmit={handleAddObservation} className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="input"
                    rows={4}
                    placeholder="What did you observe about this player?"
                    required
                  />
                </div>
            ...
            >
              <div className="p-6">
                ...
                <p className="text-navy-400 text-center mb-6">
                  Are you sure you want to delete <span className="text-white font-medium">{player.name}</span>?
                ...
                {parentInviteSuccess ? (
                  ...
                  <form onSubmit={handleInviteParent} className="space-y-4">
                    <p className="text-navy-400 text-sm">
                      Generate an invite link for {player.name}'s parent or guardian. They'll be able to view
                      the player's profile, development plan, and upcoming matches.
                    </p>
                  ...
                  <>
                    <div>
                      ...
                      <p className="text-xs text-navy-500 mt-1">
                        This message will be shown to the player and parents
                      </p>
                    </div>
                  </>
                )}
          ...
          >
            ...
            >
              <div className="p-6 border-b border-navy-800 flex items-center justify-between">
                <h2 className="font-display text-xl font-semibold text-white flex items-center gap-2">
                  <Sparkles className="w-5 h-5 text-pitch-400" />
                  Attribute Analysis for {player.name}
                </h2>
                <button
                  onClick={() => setShowAttributeAnalysis(false)}
                  className="p-2 text-navy-400 hover:text-white transition-colors"
              ...
              <div className="p-6 max-h-[70vh] overflow-y-auto">
                {analyzingAttributes ? (
                  <div className="flex flex-col items-center justify-center py-12">
                    ...
                    <p className="text-navy-400">Analyzing player attributes...</p>
