// PlayerAssistant.jsx - AI Coaching Assistant for Players/Parents
...
export default function PlayerAssistant() {
  ...
  async function loadPlayer() {
    try {
      const response = await teamService.getPlayer(playerId)
  ...
  async function loadHistory() {
    try {
      const response = await playerChatService.getHistory(playerId, 50)
  ...
  async function handleSend(messageText = input) {
    ...
    try {
      const response = await playerChatService.sendMessage(playerId, messageText.trim())
  ...
  async function clearChat() {
    ...
    try {
      await playerChatService.clearHistory(playerId)
    } catch (error) {
  ...
  if (!player) {
    return (
      <div className="p-6 max-w-4xl mx-auto text-center">
        <p className="text-navy-400">Player not found</p>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-navy-400">
                Personal assistant for {player.name}
              </p>
            </div>
          </div>
            ...
            >
              <div className="text-center mb-8">
                ...
                <p className="text-navy-400 max-w-md mx-auto">
                  ...
                  what the coaches have observed, and I'm here to help you become the best player you can be!
                </p>
              </div>
              ...
              <div className="grid grid-cols-3 gap-4 mb-8">
                <div className="card p-4 text-center">
                  ...
                  <p className="font-semibold text-white">{player.positions?.join(', ') || 'Any'}</p>
                </div>
                <div className="card p-4 text-center">
                  <Dumbbell className="w-6 h-6 text-pitch-400 mx-auto mb-2" />
                  <p className="text-xs text-navy-400">Squad #</p>
                  <p className="font-semibold text-white">{player.squad_number || '-'}</p>
                </div>
                <div className="card p-4 text-center">
      ...
      <div className="flex-shrink-0 p-4 border-t border-navy-800">
        <div className="max-w-3xl mx-auto">
          ...
          <p className="text-xs text-navy-500 mt-2 text-center">
            Your personal coach is here to help you grow as a player!
          </p>
        </div>
      </div>
    </div>
