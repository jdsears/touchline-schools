...
function buildSummaryData(pupil, label) {
  const source = pupil?.core_capabilities
...
function buildCategoryData(pupil, label, categoryKey) {
  const cat = CATEGORIES[categoryKey]
  if (!cat) return []
  const source = pupil?.[cat.source]
...
function RadarTooltip({ active, payload, label }) {
  ...
  return (
    <div className="bg-card border border-border-strong rounded-lg px-3 py-2 shadow-xl">
      <p className="text-xs text-secondary font-medium capitalize mb-1">{label}</p>
      {payload.map((entry, i) => {
...
export default function PupilRadarChart({ pupil, teamPlayers }) {
  // Default to overview if core capabilities exist, otherwise first populated detail tab
  const hasCoreCapabilities = pupil?.core_capabilities && Object.values(pupil.core_capabilities).some(v => v)
  ...
  const playerLabel = pupil.name?.split(' ')[0] || 'Pupil'

  // Build chart data based on view
  const chartData = useMemo(() => {
    const buildFn = view === 'all' ? buildSummaryData : (p, l) => buildCategoryData(p, l, view)
    let data = buildFn(pupil, playerLabel)

    for (const comp of comparisons) {
      if (compData[comp.id]) {
        const compLabel = comp.name?.split(' ')[0] || 'Pupil'
  ...
  function removeComparison(pupilId) {
    setComparisons(prev => prev.filter(c => c.id !== pupilId))
  }

  // Available pupils for comparison
  const availablePlayers = useMemo(() => {
    const selectedIds = new Set([pupil.id, ...comparisons.map(c => c.id)])
  ...
  if (!hasAttributes) {
    return (
      <div className="card p-6 text-center">
        ...
        <p className="text-secondary">No attributes analyzed yet.</p>
        <p className="text-sm text-tertiary mt-1">Use "Analyze Attributes with AI" below to generate ratings from observations.</p>
      </div>
    )
  }
  ...
  return (
    <div className="card p-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4">
        <h2 className="font-display font-semibold text-primary text-lg">Pupil Attributes</h2>
        ...
        {comparisons.length < 3 && (
          <div className="relative">
            <button
              ...
              className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-subtle text-secondary hover:text-primary hover:bg-border-default transition-colors"
            >
            ...
            {showPicker && (
              <div className="absolute top-full mt-1 left-0 z-50 bg-card border border-border-strong rounded-xl shadow-2xl w-64 overflow-hidden">
                ...
                <div className="max-h-48 overflow-y-auto">
                  {availablePlayers.length === 0 ? (
                    <p className="text-xs text-tertiary p-3 text-center">No pupils found</p>
                  ) : (
                    availablePlayers.map(p => (
                      <button
                        key={p.id}
                        onClick={() => addComparison(p)}
                        className="w-full text-left px-3 py-2 text-sm text-primary hover:bg-subtle transition-colors flex items-center gap-2"
                      >
                        <div className="w-6 h-6 rounded-full bg-border-default flex items-center justify-center text-xs font-medium text-secondary">
                          {p.name?.charAt(0)}
                        </div>
                        <span>{p.name}</span>
                        {p.squad_number && <span className="text-tertiary text-xs ml-auto">#{p.squad_number}</span>}
                      </button>
                    ))
                  )}
                </div>
      ...
      <ResponsiveContainer width="100%" height={view === 'all' ? 350 : 380}>
        <RadarChart data={chartData} cx="50%" cy="50%" outerRadius="80%">
          ...
          />
          {comparisons.map((comp, i) => {
            const label = comp.name?.split(' ')[0] || 'Pupil'
            return (
              <Radar
                key={comp.id}
                name={label}
