  ...
  return (
    <div className="space-y-3">
      ...
      {showForm && (
        <div className="bg-subtle rounded-xl p-4 space-y-3">
          <div className="flex flex-wrap gap-1.5">
            {SEVERITY_OPTS.map(s => (
              <button key={s.value} onClick={() => setForm(f => ({ ...f, severity: s.value }))}
                className={`px-2.5 py-1 rounded text-xs ${form.severity === s.value ? s.cls + ' ring-1 ring-current' : 'bg-border-default text-secondary'}`}>
                {s.label}
              </button>
            ))}
          </div>
          <div>
            <p className="text-[11px] text-tertiary mb-1">Symptoms (select all observed)</p>
          ...
          <input value={form.immediateAction} onChange={e => setForm(f => ({ ...f, immediateAction: e.target.value }))}
            placeholder="Immediate action taken" className="w-full bg-subtle border border-border-strong rounded px-2.5 py-1.5 text-sm text-primary" />
          <textarea value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))}
            placeholder="Additional notes" rows={2} className="w-full bg-subtle border border-border-strong rounded px-2.5 py-1.5 text-sm text-primary resize-none" />
      ...
      {incidents.map(inc => {
        ...
        return (
          <div key={inc.id} className="bg-card border border-border-default rounded-xl overflow-hidden">
            <button onClick={() => loadDetail(inc.id)} className="w-full p-3 flex items-center gap-3 text-left">
              ...
              <span className="text-sm text-secondary">{new Date(inc.occurred_at).toLocaleDateString('en-GB')}</span>
              {inc.match_opponent && <span className="text-xs text-tertiary">vs {inc.match_opponent}</span>}
              <div className="ml-auto">{expanded ? <ChevronUp className="w-4 h-4 text-tertiary" /> : <ChevronDown className="w-4 h-4 text-tertiary" />}</div>
            </button>
            {expanded && detail && (
              <div className="px-3 pb-3 space-y-2 border-t border-border-default">
                ...
                {detail.incident.notes && <p className="text-xs text-secondary">{detail.incident.notes}</p>}
                <div className="space-y-1 mt-2">
                  <p className="text-[11px] text-tertiary font-medium uppercase tracking-wider">Return-to-Play Protocol</p>
                  {detail.followups.map(f => {
                    ...
                    return (
                      <div key={f.stage} className="flex items-center gap-2 text-xs py-1">
                        ...
                        {f.completed_at && <span className="ml-auto text-[10px] text-tertiary">{new Date(f.completed_at).toLocaleDateString('en-GB')}</span>}
                      </div>
                    )
                  })}
                </div>
      ...
      {incidents.length === 0 && !showForm && (
        <p className="text-xs text-tertiary text-center py-2">No concussion incidents recorded.</p>
      )}
    </div>
  )
}
