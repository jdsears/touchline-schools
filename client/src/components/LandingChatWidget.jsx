    ...
    <div className="fixed bottom-6 right-6 z-50">
      <AnimatePresence>
        {isOpen && !isMinimized && (
          <motion.div
            ...
            className="absolute bottom-16 right-0 w-[380px] max-w-[calc(100vw-3rem)] bg-card rounded-2xl shadow-2xl border border-border-strong overflow-hidden"
          >
            {/* Header */}
            <div className="bg-gradient-to-r from-pitch-600 to-pitch-500 px-4 py-3 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-primary font-medium text-sm">Ask about MoonBoots Sports</p>
                  <p className="text-primary/70 text-xs">We're here to help</p>
                </div>
              </div>
                  ...
                  <div className="space-y-2">
                    {suggestedQuestions.map((q) => (
                      <button
                        ...
                        className="w-full text-left px-3 py-2 rounded-lg bg-subtle text-sm text-secondary hover:text-primary hover:bg-subtle transition-colors"
                      >
                        {q}
                      </button>
                    ))}
            ...
            <div className="p-3 border-t border-border-default">
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-subtle border border-border-strong rounded-xl px-3 py-2 text-sm text-primary placeholder:text-tertiary focus:outline-none focus:border-pitch-500"
                />
                <button
                  onClick={() => handleSend()}
                  disabled={!input.trim() || loading}
                  className="p-2 rounded-xl bg-pitch-600 text-on-dark disabled:opacity-50 disabled:cursor-not-allowed hover:bg-pitch-500 transition-colors"
                >
      ...
      <motion.button
        ...
        className={`
          ...
          ${isOpen && !isMinimized
            ? 'bg-subtle hover:bg-border-default'
