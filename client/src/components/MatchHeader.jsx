...
import { resultOutcome, scoreline, scoreVocab, hasRecordedResult } from '../lib/results'
...
export default function MatchHeader({ match, team }) {
  ...
  const sport = team.sport || match.sport
  const vocab = scoreVocab(sport)
  const hasResult = hasRecordedResult(match)
  const isHome = match.home_away === 'home'
  const isPast = (match.date || match.match_date) && new Date(match.date || match.match_date) < new Date()
  const state = isPast || hasResult ? 'post' : 'pre'
  ...
  const resultChar = resultOutcome(match, sport)
  // Innings sports show runs/wickets under the headline, e.g. "120/4 (20 ov)"
  const schema = getResultSchema(String(sport || '').toLowerCase())
  const inningsLine = hasResult && vocab.innings && schema?.summarise && match.result_data
    ? [schema.summarise(isHome ? match.result_data.for : match.result_data.against), schema.summarise(isHome ? match.result_data.against : match.result_data.for)]
        .map(s => s || '—').join('  ·  ')
    : null
          ...
          <div className="flex flex-col items-center gap-1.5 min-w-[100px]">
            {state === 'pre' ? (
              ...
              <>
                ...
                <span className="text-[11px] tracking-[0.08em] uppercase font-semibold" style={{ color: onNavyDim }}>
                  {inningsLine || vocab.unit}
                </span>
              </>
            ) : (
